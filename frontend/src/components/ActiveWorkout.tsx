...
import { ChevronDown, CheckCircle2, Play, Pause, FastForward, RotateCcw, Image as ImageIcon, Minus, Plus } from 'lucide-react';
...
export default function ActiveWorkout({ session, onUpdate, onClose, onFinish }: ActiveWorkoutProps) {
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-900/50">
          ...
          {guide && (
            <div className="space-y-4">
              {guide.mediaUrls?.[0] ? (
                 <video src={guide.mediaUrls[0]} autoPlay loop muted playsInline className="w-full h-48 object-cover rounded-2xl bg-slate-800" />
              ) : (
              ...
              {guide.instructions && (
                <div className="text-sm text-slate-300 leading-relaxed bg-slate-800/50 p-4 rounded-xl border border-slate-700/50">
                  {guide.instructions}
                </div>
              )}
            </div>
  ...
  const renderSetInput = (s: SetLog, index: number, isCurrent: boolean) => {
    const [w, setW] = useState(s.weight.toString());
    const [r, setR] = useState(s.reps.toString());

    return (
      <div key={index} className={`flex items-center gap-3 p-3 rounded-xl transition-colors ${s.completed ? 'bg-slate-900/50 opacity-60' : isCurrent ? 'bg-slate-800 border-l-4 border-blue-500 shadow-lg' : 'bg-slate-900/80'}`}>
        ...
        {!s.completed ? (
          <button onClick={() => handleCompleteSet(index, parseFloat(w) || 0, parseInt(r) || 0)} className="w-12 h-11 flex items-center justify-center bg-blue-600 hover:bg-blue-500 rounded-lg text-white shadow-md">
            <CheckCircle2 size={20} />
          </button>
        ) : (
          <button onClick={() => {
            const ex = session.exercises[itemIndex];
            const newSets = ex.sets.map((set, i) => i === index ? { ...set, completed: false } : set);
            onUpdate({ ...session, exercises: session.exercises.map((e, i) => i === itemIndex ? { ...e, sets: newSets, checked: false } : e) });
          }} className="w-12 h-11 flex items-center justify-center bg-slate-800 hover:bg-slate-700 rounded-lg text-slate-400">
            <RotateCcw size={16} />
          </button>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-slate-900/50 pb-8">
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="px-2 py-1 bg-blue-500/20 text-blue-400 text-xs font-bold rounded uppercase tracking-wider">{ex.tier}</div>
            </div>
          ...
          {guide && (
            <div className="space-y-4">
              ...
              {guide.instructions && (
                <div className="text-sm text-slate-300 leading-relaxed bg-slate-800/50 p-4 rounded-xl border border-slate-700/50 whitespace-pre-line">
                  {guide.instructions.replace(/\n/g, '\n\n')}
                </div>
              )}
            </div>
          )}
        ...
        <div className="bg-slate-950 p-4 rounded-t-3xl border-t border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-10">
          <div className="max-h-[40vh] overflow-y-auto space-y-2 pb-6 px-2">
            ...
            {ex.sets.map((s, i) => renderSetInput(s, i, i === currentSetIndex))}
